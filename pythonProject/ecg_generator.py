...
'''def get_ecg_records(database,channel):
    ...
    for record in record_files:
        if record in useless_afrecord:
            ...
            print('processing record:',record)
            sig = wfdb.rdrecord(record_name=record,channels =[channel],pn_dir=database)
            data = sig.p_signal.flatten()
...
nsr_train = get_ecg_records('nsrdb', 0)
af_train = get_ecg_records('afdb',0)
...
noised_nsr = ecg_generator('nsr',nsr,wgn_nsr,ma,bw,win_size=1280,batch_size=32)
noised_af  = ecg_generator('af',af,wgn_af,ma,bw,win_size=1280,batch_size=32)
